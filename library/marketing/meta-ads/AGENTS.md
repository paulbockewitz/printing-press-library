# Meta Ads Printed CLI Agent Guide

This directory is a generated `meta-ads-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
meta-ads-pp-cli which "<capability>" --json
meta-ads-pp-cli <command> --help
```
...
```bash
meta-ads-pp-cli <command> --help
meta-ads-pp-cli <command> --dry-run --agent
```
