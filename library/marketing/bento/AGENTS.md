# Bento Printed CLI Agent Guide

This directory is a generated `bento-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
bento-pp-cli doctor --json
bento-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
bento-pp-cli which "<capability>" --json
bento-pp-cli <command> --help
```
...
```bash
bento-pp-cli <command> --help
bento-pp-cli <command> --dry-run --agent
```
