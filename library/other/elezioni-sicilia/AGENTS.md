# Elezioni Sicilia Printed CLI Agent Guide

This directory is a generated `elezioni-sicilia-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
elezioni-sicilia-pp-cli doctor --json
elezioni-sicilia-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
elezioni-sicilia-pp-cli which "<capability>" --json
elezioni-sicilia-pp-cli <command> --help
```
...
```bash
elezioni-sicilia-pp-cli <command> --agent
```
...
```bash
elezioni-sicilia-pp-cli <command> --help
elezioni-sicilia-pp-cli <command> --dry-run --agent
```
