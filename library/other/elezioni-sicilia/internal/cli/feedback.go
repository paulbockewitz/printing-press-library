...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".elezioni-sicilia-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ELEZIONI_SICILIA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ELEZIONI_SICILIA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "elezioni-sicilia-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.elezioni-sicilia-pp-cli/feedback.jsonl.
When ` + "`ELEZIONI_SICILIA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ELEZIONI_SICILIA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  elezioni-sicilia-pp-cli feedback list
  elezioni-sicilia-pp-cli feedback list --limit 5
  elezioni-sicilia-pp-cli feedback list --json`,
