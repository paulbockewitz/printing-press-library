# PCGS Printed CLI Agent Guide

This directory is a generated `pcgs-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
pcgs-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
pcgs-pp-cli which "<capability>" --json
pcgs-pp-cli <command> --help
```
...
```bash
pcgs-pp-cli <command> --help
pcgs-pp-cli <command> --dry-run --agent
```
