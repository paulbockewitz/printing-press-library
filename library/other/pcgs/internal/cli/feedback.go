...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".pcgs-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PCGS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PCGS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "pcgs-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.pcgs-pp-cli/feedback.jsonl.
When ` + "`PCGS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PCGS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  pcgs-pp-cli feedback list
  pcgs-pp-cli feedback list --limit 5
  pcgs-pp-cli feedback list --json`,
