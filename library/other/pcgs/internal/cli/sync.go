...
Resource scoping:
  ...
  pcgs-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  pcgs-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  pcgs-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  pcgs-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  pcgs-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/pcgs-pp-cli/data.db)")
