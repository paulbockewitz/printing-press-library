...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	// them by message text would clobber the more-specific code.
	var typed *cliError
	if As(err, &typed) {
		return err
	}
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'pcgs-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your token. Set it with: pcgs-pp-cli auth set-token <token>"+
			"\n      or: export PCGS_AUTH_TOKEN=<your-token>"+
			"\n      Run 'pcgs-pp-cli doctor' to check auth status.", err))
