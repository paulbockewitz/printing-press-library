...
type Client struct {
	...
	// PATCH: optional per-request logging callback for lookup_log wiring.
	logHook func(LookupEvent)
}

// PATCH: event payload for store lookup_log writes from CLI root hook wiring.
// LookupEvent is what gets passed to a Client.LogHook on every request.
...
// PATCH: install request logging callback; hook failures never affect API result.
// SetLogHook installs a callback invoked once per request (live OR cache hit).
func (c *Client) SetLogHook(h func(LookupEvent)) {
	c.logHook = h
}
...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "pcgs-pp-cli", "http")
...
func (c *Client) GetWithHeaders(path string, params map[string]string, headers map[string]string) (json.RawMessage, error) {
	...
	if !c.NoCache && !c.DryRun && c.cacheDir != "" {
		if cached, ok := c.readCache(path, params); ok {
			if c.logHook != nil {
				c.logHook(LookupEvent{
					Endpoint:       path,
					Method:         http.MethodGet,
					Params:         params,
					RequestHash:    c.cacheKey(path, params),
					HTTPStatus:     0,
					IsValidRequest: true,
					ServerMessage:  "",
					DurationMs:     0,
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "pcgs-pp-cli/0.1.0")
		}
		...
		if resp.StatusCode < 400 {
			...
			if c.logHook != nil {
				isValid, msg := parseLookupEnvelope(respBody)
				c.logHook(LookupEvent{
					Endpoint:       path,
					Method:         method,
					Params:         params,
...
func parseLookupEnvelope(b []byte) (bool, string) {
	...
	isValid := true
	if env.IsValidRequest != nil {
		isValid = *env.IsValidRequest
	}
	return isValid, env.ServerMessage
}
