...
Resource scoping:
  ...
  delta-trip-pp-cli sync

  # Sync specific resources only
  delta-trip-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  delta-trip-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  delta-trip-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  delta-trip-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  delta-trip-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("delta-trip-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/delta-trip-pp-cli/data.db)")
