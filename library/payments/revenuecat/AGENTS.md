# RevenueCat Printed CLI Agent Guide

This directory is a generated `revenuecat-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
revenuecat-pp-cli doctor --json
revenuecat-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
revenuecat-pp-cli which "<capability>" --json
revenuecat-pp-cli <command> --help
```
...
```bash
revenuecat-pp-cli <command> --agent
```
...
```bash
revenuecat-pp-cli <command> --help
revenuecat-pp-cli <command> --dry-run --agent
```
