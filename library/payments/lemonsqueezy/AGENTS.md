# Lemon Squeezy Printed CLI Agent Guide

This directory is a generated `lemonsqueezy-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
lemonsqueezy-pp-cli doctor --json
lemonsqueezy-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
lemonsqueezy-pp-cli which "<capability>" --json
...
```bash
lemonsqueezy-pp-cli <command> --agent
```
...
```bash
lemonsqueezy-pp-cli <command> --help
lemonsqueezy-pp-cli <command> --dry-run --agent
```
