# AnkiWeb Printed CLI Agent Guide

This directory is a generated `ankiweb-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
ankiweb-pp-cli doctor --json
ankiweb-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
ankiweb-pp-cli which "<capability>" --json
ankiweb-pp-cli <command> --help
```
...
```bash
ankiweb-pp-cli <command> --help
ankiweb-pp-cli <command> --dry-run --agent
```
