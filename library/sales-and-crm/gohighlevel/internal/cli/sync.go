...
Resource scoping:
  ...
  gohighlevel-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  gohighlevel-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  gohighlevel-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  gohighlevel-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  gohighlevel-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/gohighlevel-pp-cli/data.db)")
...
// syncFetcher is the interface syncResource needs from a client. It covers
// PATCH(amend-2026-05-20: contacts + tags sync) — widened from the original
// (Get + RateLimit) so resources whose list endpoint is POST (GHL contacts
// search) can be synced through the same loop. PostWithParams returns
// (data, statusCode, err); the wrapper in fetchSyncPage maps that to the
// (data, err) shape the rest of syncResource expects.
...
//
// PATCH(amend-2026-05-20: contacts + tags sync) — interface widened to
// syncFetcher (was inline Get+RateLimit) so POST-list endpoints work.
func syncResource(c syncFetcher, db *store.Store, resource, sinceTS string, full bool, maxPages int, latestOnly bool, userParams *syncUserParams) syncResult {
	started := time.Now()
	...
	// PATCH(amend-2026-05-20: contacts + tags sync) — fill `{locationId}` in
	// path templates from --global-param locationId=<id> (or GHL_LOCATION_ID
	...
	for {
		...
		// endpoint is POST (GHL contacts /contacts/search) build a JSON body
		// from params (locationId, pageLimit, searchAfter) and POST it. The
		// searchAfter cursor is JSON-encoded as a single string for storage
		// in sync_state.last_cursor and decoded back into an array here.
		data, err := fetchSyncPage(c, resource, path, params, cursor, pageSize)
		if err != nil {
		...
		// PATCH(amend-2026-05-20: contacts + tags sync) — derive
		// contacts_tags rows from each contacts page so downstream automation
		// can query per-(contact, tag) application data via SQL. Each row
		// has composite id "<contactId>:<tag>" and uses the contact's
		...
		if resource == "contacts" {
			if err := deriveContactsTagsFromPage(db, items); err != nil {
				if humanFriendly {
					...
					fmt.Fprintf(os.Stdout, `{"event":"sync_warning","resource":"contacts_tags","reason":"derivation_failed","message":"%s"}`+"\n", strings.ReplaceAll(err.Error(), `"`, `\"`))
				}
			}
		}
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "free_slots":
		return db.UpsertFreeSlots(data)
	case "notes":
		return db.UpsertNotes(data)
	case "contacts_tags":
		return db.UpsertContactsTags(data)
	case "tasks":
	...
	case "custom_values":
		return db.UpsertCustomValues(data)
	case "locations_tags":
		return db.UpsertLocationsTags(data)
	case "status":
...
// syncResourceMethod returns the HTTP method to use when syncing a resource.
// PATCH(amend-2026-05-20: contacts + tags sync) — added so POST-list endpoints
...
func resolveSyncLocationID(p *syncUserParams, resource string) string {
	if p != nil {
		...
		if v, ok := p.flatGlobal["locationId"]; ok && v != "" {
			return v
		}
	}
	if v := os.Getenv("GHL_LOCATION_ID"); v != "" {
		return v
	}
	return ""
}
...
// on the per-resource method registry. PATCH(amend-2026-05-20: contacts +
// tags sync) — added so POST-list endpoints work with the same syncResource
// loop. For POST, params is rewritten into a JSON body and a separate body
// for searchAfter cursor handling (GHL returns searchAfter as an array; we
// JSON-encode it as a single string for storage in sync_state.last_cursor
// and decode it back into an array here).
func fetchSyncPage(c syncFetcher, resource, path string, params map[string]string, cursor string, pageSize paginationDefaults) (json.RawMessage, error) {
	if syncResourceMethod(resource) != "POST" {
		return c.Get(path, params)
	}
	// POST path. Build a JSON body from params; locationId comes from the
	// caller (params via --global-param or env), pageLimit from pageSize,
	// searchAfter from the cursor (stored as a JSON-encoded array string
	// in sync_state.last_cursor).
	body := map[string]any{}
	if v, ok := params["locationId"]; ok && v != "" {
		body["locationId"] = v
	}
	if cursor != "" {
		var sa []any
		if err := json.Unmarshal([]byte(cursor), &sa); err == nil {
			body["searchAfter"] = sa
		} else {
			// Cursor wasn't a JSON array — fall back to treating it as a
			// single-element string array (defensive; the only writer is
			// extractSearchAfterCursor below, which always emits an array).
			body["searchAfter"] = []any{cursor}
		}
	}
	body["pageLimit"] = pageSize.limit
	// Pass-through for any extra POST body fields the user set via
	// --resource-param resource:query=foo. Reject pagination keys to keep
	// resume state consistent.
	for k, v := range params {
		if k == "locationId" || k == pageSize.limitParam || k == pageSize.cursorParam {
			continue
		}
		body[k] = v
	}
	data, _, err := c.PostWithParams(path, nil, body)
	return data, err
}

// extractSearchAfterCursor pulls the `searchAfter` array from the last item
// of the page (GHL /contacts/search returns each contact's lexicographic
// sort cursor in `searchAfter` on the response envelope, OR the last
// contact's `id` works as the next-page anchor). PATCH(amend-2026-05-20:
// contacts + tags sync) — separate cursor extractor because the standard
// extractPaginationFromEnvelope assumes a string cursor; GHL emits an
// array. Returns the JSON-encoded array as a string for storage.
func extractSearchAfterCursor(data json.RawMessage) (string, bool) {
	var envelope map[string]json.RawMessage
	if err := json.Unmarshal(data, &envelope); err != nil {
		return "", false
	}
	// Strategy 1: response carries a top-level searchAfter cursor.
	if raw, ok := envelope["searchAfter"]; ok {
		var arr []any
	...
	if err := json.Unmarshal(rawContacts, &contacts); err != nil || len(contacts) == 0 {
		return "", false
	}
...
var resourceIDFieldOverrides = map[string]string{
	"locations": "id",
	// PATCH(amend-2026-05-20: contacts + tags sync) — new resources.
	// contacts_tags rows use a composite "<contactId>:<tag>" id assembled
	// at derivation time (deriveContactsTagsFromPage); the "id" override
	// here is a no-op for that path (extractID hits the synthetic field)
	// but keeps the registry honest for any future direct upserts.
	"contacts":       "id",
	"locations_tags": "id",
	"contacts_tags":  "id",
}
...
// deriveContactsTagsFromPage fans out a contacts page into one
...
func deriveContactsTagsFromPage(db *store.Store, items []json.RawMessage) error {
	...
	for _, item := range items {
		...
		if contactID == "" {
			// No id; can't anchor the tag row. Skip silently — the
			// parent contacts upsert will have logged a primary_key
			// miss already.
			continue
		}
		...
		for _, t := range tagsArr {
			...
			row := map[string]any{
				...
				"tag":         tag,
				"tagged_at":   dateUpdated,
				"source":      "derived_from_contacts_search",
			}
