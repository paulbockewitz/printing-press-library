...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "tella-pp-cli",
		Short: `Tella CLI — Every Tella API operation behind one CLI, with a local SQLite store, FTS5 transcript search, and webhook tooling that r…`,
		Long: `Tella CLI — Every Tella API operation behind one CLI, with a local SQLite store, FTS5 transcript search, and webhook tooling that r…
...
Health check: run 'tella-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("tella-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'tella-pp-cli profile list')")
	...
	rootCmd.AddCommand(newExportsCmd(flags))
