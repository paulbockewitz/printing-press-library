# Printify Printed CLI Agent Guide

This directory is a generated `printify-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
printify-pp-cli doctor --json
printify-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
printify-pp-cli which "<capability>" --json
printify-pp-cli <command> --help
```
...
```bash
printify-pp-cli <command> --help
printify-pp-cli <command> --dry-run --agent
```
