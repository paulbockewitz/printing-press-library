# UCP Printed CLI Agent Guide

This directory is a generated `ucp-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
ucp-pp-cli doctor --json
ucp-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
ucp-pp-cli which "<capability>" --json
ucp-pp-cli <command> --help
```
...
```bash
ucp-pp-cli <command> --help
ucp-pp-cli <command> --dry-run --agent
```
