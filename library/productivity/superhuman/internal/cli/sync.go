...
Exit codes & warnings:
  ...
  superhuman-pp-cli sync

  # Sync specific resources only
  superhuman-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  superhuman-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  superhuman-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  superhuman-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  superhuman-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/superhuman-pp-cli/data.db)")
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"teams": "/v3/teams.suggest?includeSingletons=true",
	}
