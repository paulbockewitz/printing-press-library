...
// isTerminalReader is the stdin-side counterpart of isTerminal. Confirmation
// prompts use it to refuse to prompt when stdin is piped: prompting in that
// case either blocks forever or silently reads script bytes as the answer.
...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'superhuman-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your token. Set it with: superhuman-pp-cli auth set-token <token>"+
			"\n      or: export SUPERHUMAN_JWT=<your-token>"+
			"\n      Run 'superhuman-pp-cli doctor' to check auth status.", err))
