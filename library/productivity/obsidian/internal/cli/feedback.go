...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".obsidian-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("OBSIDIAN_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("OBSIDIAN_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/productivity/obsidian/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.obsidian-pp-cli/feedback.jsonl.
When ` + "`OBSIDIAN_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`OBSIDIAN_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  obsidian-pp-cli feedback list
  obsidian-pp-cli feedback list --limit 5
  obsidian-pp-cli feedback list --json`,
