...
In local mode: searches locally synced data only.`,
  ...
  obsidian-pp-cli search "error timeout"

  # Force local search only
  obsidian-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  obsidian-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  obsidian-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /live-search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get("/live-search", map[string]string{
					"query": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'obsidian-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
