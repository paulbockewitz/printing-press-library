# Obsidian Printed CLI Agent Guide

This directory is a generated `obsidian-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
obsidian-pp-cli doctor --json
obsidian-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
obsidian-pp-cli which "<capability>" --json
obsidian-pp-cli <command> --help
```
...
```bash
obsidian-pp-cli <command> --help
obsidian-pp-cli <command> --dry-run --agent
```
