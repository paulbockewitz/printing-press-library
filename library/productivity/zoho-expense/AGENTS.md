# Zoho Expense Printed CLI Agent Guide

This directory is a generated `zoho-expense-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
zoho-expense-pp-cli doctor --json
zoho-expense-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
zoho-expense-pp-cli which "<capability>" --json
zoho-expense-pp-cli <command> --help
```
...
```bash
zoho-expense-pp-cli <command> --agent
```
...
```bash
zoho-expense-pp-cli <command> --help
zoho-expense-pp-cli <command> --dry-run --agent
```
