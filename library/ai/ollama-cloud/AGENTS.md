# Ollama Cloud Printed CLI Agent Guide

This directory is a generated `ollama-cloud-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
ollama-cloud-pp-cli doctor --json
ollama-cloud-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
ollama-cloud-pp-cli which "<capability>" --json
ollama-cloud-pp-cli <command> --help
```
...
```bash
ollama-cloud-pp-cli <command> --agent
```
...
```bash
ollama-cloud-pp-cli <command> --help
ollama-cloud-pp-cli <command> --dry-run --agent
```
